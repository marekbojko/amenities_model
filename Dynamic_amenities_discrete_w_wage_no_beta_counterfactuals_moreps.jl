...
using CSV, Plots, Plots.PlotMeasures, LineSearches
...
# Save parameters
io = open("params/"*string(J)*"_"*string(K)*"_"*string(S)*"dynamic_w_wage_no_beta_counterfactuals_moreeps_discrete.txt", "w")
write(io, string(P))
close(io)
...
# initial guess
initial_x = log.([3.1*ones(P.J); 18*ones(P.J); 5*ones(P.J*(P.S-1))])
...
results = optimize(f_obj, x_min_NM, method = LBFGS(; linesearch = LineSearches.HagerZhang()); autodiff = :forward, iterations = 5*10^6,
                        x_tol = 1e-32, f_tol = 1e-32, g_tol = 1e-32)
@show results
io = open("optim_output/"*string(J)*"_"*string(K)*"_"*string(S)*"_NM_dynamic_w_wage_no_beta_counterfactuals_moreeps.txt", "w")
