...
value_function_iteration_param = (tol = 10^(-8),
                                  max_iter = 10^5)
...
# Save parameters
io = open("params/"*string(J)*"_"*string(K)*"_"*string(S)*"dynamic_wage_rand_MC.txt", "w")
write(io, string(P))
close(io)
...
savefig("optim_output/housing_demand_r_"*string(J)*"_"*string(K)*"_"*string(S)*"_NM_dynamic_wage_rand_MC.png")
...
plot!(twinx(),r_eq[sortperm(get_ordered_indices(P.delta_j))],color=:green,xticks=:none,label="r",right_margin = 10mm)
savefig("optim_output/eq_amenities_r_"*string(J)*"_"*string(K)*"_"*string(S)*"_NM_dynamic_wage_rand_MC.png")
...
savefig("optim_output/welfare_"*string(J)*"_"*string(K)*"_"*string(S)*"_NM_dynamic_wage_rand_MC.png")
...
savefig("optim_output/welfare_landlords_"*string(J)*"_"*string(K)*"_"*string(S)*"_NM_dynamic_wage_rand_MC.png")
