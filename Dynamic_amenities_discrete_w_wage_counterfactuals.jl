...
# Save parameters
io = open("params/"*string(J)*"_"*string(K)*"_"*string(S)*"dynamic_w_wage_counterfactuals_discrete.txt", "w")
write(io, string(P))
close(io)
...
@show results
io = open("optim_output/"*string(J)*"_"*string(K)*"_"*string(S)*"_NM_dynamic_w_wage_counterfactuals.txt", "w")
